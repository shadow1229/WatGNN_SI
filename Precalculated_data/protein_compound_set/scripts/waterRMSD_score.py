...
def run(ref_fn, mod_fn, verbose=False, n_pred=[],label=None):
    use_model = False
    refp,refw,bfacs      = read_pdb(ref_fn,cutoff_max = 40.0)
    ...
    lt_15= np.where(dist<1.5)[0]  
    lt_10= np.where(dist<1.0)[0]  
    ...
    if verbose:
        ...
        for k in range(len(n_pred)):
            sys.stdout.write("%6d %6d %6.3f | %6.3f %6.3f %6.3f  %6d %6d %6d  %6.3f\n"%\
                    (n_wat, n_pred[k], cuts[k], rmsd[k], mdev[k], medd[k], TP_05[k], TP_10[k], TP_15[k] , dev_20[k]))
    #
